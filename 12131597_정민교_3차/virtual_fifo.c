...
struct frame frame_table[128]={0,0};
...
int main(int argc, char *arg[])
{
	...
	char buffer[256];
	FILE *fp;
	FILE *wfp = fopen("physical_fifo.txt","w");
		...
		if(i >127)//swap
		{
			...
			if(check ==0) // TLB fault
			{
				...
				else	//fault
				{
					...
					for(k=0; k<256; k++)
					{
						if(page_table[k].frame == j)
						{
							if(k != num/256)
							{
								for(m=0; m<16; m++)
								{
									if(TLB_t[m].page_num ==k)
									{
										TLB_t[m].frame_num=-1;
										page_table[TLB_t[m].page_num].count=1000000;
									}
								}
								page_table[k].frame=-1;
								frame_table[page_table[k].frame].valid = 0;
								j++;
								break;
							}
						}
					...
					TLB_t[maxpt].page_num = num/256;
					TLB_t[maxpt].frame_num = page_table[num/256].frame;
					page_table[TLB_t[maxpt].page_num].count=0;
				}
			}
			check=0;
			max =-1;
